...
@group(2) @binding(0) var materialCube: texture_cube<f32>;
@group(3) @binding(0) var materialSampler: sampler;
...
fn fs_main(fragData: VertexOut) -> @location(0) vec4f {
  ...
  let skybox = textureSample(materialCube, materialSampler, reflectDir * vec3f(1, 1, -1)).rgb;
